...
# Output directory
output_dir = "/home/wfarz001/PhD_candidacy_exam/hybrid_compare_multi-gamma"
...
def run_hybrid_tumor_simulation(P_gamma=0.6, days=120, N_p=50):
    """
    ...
    #fig1.suptitle(f'Hybrid Tumor-Drug Model Results (P_? = {P_gamma}, {days} days)', fontsize=16)
    
    fig2 = simulator.plot_detailed_analysis()
    #fig2.suptitle(f'Detailed Analysis (P_? = {P_gamma})', fontsize=16)
...
###################### Function for comparison across  P_gamma_values #######################################################################################   
    
def compare_drug_effectiveness():
    """Compare different drug effectiveness levels"""
    P_gamma_values = [1, 0.05, 0.3, 0.6]
    results = {}
    
    print("=== Comparing Drug Effectiveness Levels ===")
    
    for P_gamma in P_gamma_values:
    
        print(f"\nRunning simulation with P_gamma = {P_gamma}")
        
        simulator = run_hybrid_tumor_simulation(P_gamma=P_gamma, days=120, N_p=50)
        
        results[P_gamma] = {
            'radius_history': simulator.ca_model.radius_history.copy(),
            'invasive_radius_history': simulator.ca_model.invasive_radius_history.copy(),
    ...
    for i, P_gamma in enumerate(P_gamma_values):
        radius_cm = [r / 10.0 for r in results[P_gamma]['radius_history']]
        plt.plot(results[P_gamma]['time_history'], radius_cm, 
                color=colors[i], linewidth=2, label=f'P_gamma = {P_gamma}', marker='o', markersize=3)
    
    plt.xlabel('Time (days)', fontsize=12)
    plt.ylabel('Proliferative Tumor Radius (cm)', fontsize=12)
    plt.title('Tumor Growth Under Different P_gamma ', fontsize=14)
    plt.legend(fontsize=11)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/plot_compare_P_gamma.png", dpi=300)
    #plt.show()
    
    # Print summary
    print("\n=== Summary of Results ===")
    print("P_gamma\tFinal Proliferative Radius (cm)\tFinal Invasive Radius (cm)\tInvasive Cells\tProliferative Cells")
    print("-" * 60)
    for P_gamma in P_gamma_values:
        final_radius_cm = results[P_gamma]['radius_history'][-1] / 10.0
        final_invasive_radius_cm = results[P_gamma]['invasive_radius_history'][-1] / 10.0
        invasive = results[P_gamma]['final_invasive']
        prolif = results[P_gamma]['final_prolif']
        print(f"{P_gamma}\t{final_radius_cm:.2f}\t{final_invasive_radius_cm:.2f}\t\t{invasive}\t\t{prolif}")
...
if __name__ == "__main__":
    ...
    #simulator = run_hybrid_tumor_simulation(P_gamma=0.6, days=120, N_p=75)
    
    # comparison study
    results = compare_drug_effectiveness()
